...
class TestCase(object):
    ...
    * maxDiff: sets the maximum length of a diff in failure messages
    ...
    def __init__(self, methodName='runTest'):
        ...
        self._cleanups = []
        # self._filePath = testMethod.__file__  # 添加case所在目录路径
        # print os.getcwd()
    ...
    def run(self, result=None):
        ...
        if (getattr(self.__class__, "__unittest_skip__", False) or
            ...
            return
        try:
            startTime = datetime.datetime.now()    # 添加每条用例执行时间的开始时间
            success = False
            try:
                self.setUp()
            ...
            if self._testMethodDoc:
                self._testMethodDoc = self._testMethodDoc + '<br />执行耗时：' + str(caseRunTime)
            else:
                self._testMethodDoc = '<br />执行耗时：' + str(caseRunTime)
    ...
    def add(func):
        def wrapper(self, first, second, msg=None):
            try:
                ...
                now = nowSec + nowMsec
                path = 'D:/AutoTestReport/Program/ScreenShot/'

                if not os.path.exists(path):
                    os.makedirs(path)
                os.popen("adb wait-for-device")
                os.popen("adb shell screencap -p /data/local/tmp/tmp.png")
                os.popen("adb pull /data/local/tmp/tmp.png " + path + now + "_shot.png")
                os.popen("adb shell rm /data/local/tmp/tmp.png")
                print 'ScreenShot Path:' + path + now + "_shot.png"
