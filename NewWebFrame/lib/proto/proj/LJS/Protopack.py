...
class C2S_ACCNAME():
    ...
    def c2s_login(self, paramlist):
        ...
        登录协议
        :str token: 登录账号
        :int platformID: 平台号,不填是0
        :str channelID: 渠道名字
        :str deviceIMEI: 设备号
        :str client: IOS、ANDROID、PC
        """
        c2s = accname_pb2.c2s_login()
        c2s.token = paramlist[0]
        c2s.platformID = paramlist[1]
        c2s.channelID = paramlist[2]
        c2s.deviceIMEI = paramlist[3]
        c2s.client = paramlist[4]
        return c2s
...
class C2S_ACTIVE():
    ...
    def c2s_get_active_open_state(self):
        """
        获取开启的活动
        :param:  不需要填写参数
        """
        c2s = active_pb2.c2s_get_active_open_state()
        return c2s

    def c2s_get_seven_achieve_info(self):
        """
        获取开服七日成就的活动
        :param:  不需要填写参数
        """
        c2s = active_pb2.c2s_get_seven_achieve_info()
        return c2s

    def c2s_get_seven_achieve_reward(self, paramlist):
        """
        获取开服七日成就的奖励
        :int id: 奖励id  
        """
        c2s = active_pb2.c2s_get_seven_achieve_reward()
        c2s.id = paramlist[0]
        return c2s

    def c2s_get_daily_online_info(self):
        """
        获取每日在线的活动
        :param:  不需要填写参数
        """
        c2s = active_pb2.c2s_get_daily_online_info()
        return c2s

    def c2s_get_daily_online_reward(self, paramlist):
        """
        获取每日在线的活动
        :int id: 奖励id  
        """
        c2s = active_pb2.c2s_get_daily_online_reward()
        c2s.id = paramlist[0]
        return c2s
...
class C2S_PVE():
    ...
    def c2s_pve_battle_reward(self, paramlist):
        ...
        领取pve战斗奖励
        :int battle_id:  战斗ID
        :int partner_unique_ids:  伙伴UID(可选)
        :int partner_unique_ids:  伙伴UID(可选)
        """
        c2s = pve_pb2.c2s_pve_battle_reward()
        battle = c2s.battle.add()
        battle.battle_id = paramlist[0]
        if len(paramlist) > 1:
            battle.partner_unique_ids.extends(paramlist[1:])
        return c2s
...
class C2S_TRIAL():
    ...
    def c2s_trial_battle_result_confirm(self):
        """
        前端确认试炼战斗结束
        :param: 不需要填写参数
        """
        c2s = trial_pb2.c2s_trial_battle_result_confirm()
        return c2s
